...
def main(args):
    ...
    # Setup optimizer
    vq_model.encoder.requires_grad_(False)
    vq_model.quantize.requires_grad_(False)
    vq_model.quant_conv.requires_grad_(False)
    optimizer = torch.optim.Adam(filter(lambda p: p.requires_grad, vq_model.parameters()), lr=args.lr, betas=(args.beta1, args.beta2))
    ...
    if args.vq_ckpt:
        ...
        del checkpoint
        logger.info(f"Training from checkpoint: {args.vq_ckpt}")
    ...
    qzshape = [8, args.codebook_embed_dim, 32, 32]
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for image_code, imgs, _ in loader:
            imgs = imgs.to(device, non_blocking=True).flatten(0, 1).permute(0, 3, 1, 2) / 255
            image_code = image_code.to(device, non_blocking=True).flatten(0, 1)

            # generator training
            optimizer.zero_grad()
            with torch.cuda.amp.autocast(dtype=ptdtype):  
                recons_imgs = vq_model(image_code, True, qzshape)
                recons_imgs = torch.clamp(127.5 * recons_imgs + 128.0, 0, 255) / 255

                loss_gen = vq_loss(None, imgs, recons_imgs, optimizer_idx=0, global_step=train_steps+1, 
            ...
            with torch.cuda.amp.autocast(dtype=ptdtype):
                loss_disc = vq_loss(None, imgs, recons_imgs, optimizer_idx=1, global_step=train_steps+1,
                                    logger=logger, log_every=args.log_every)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--obj-pkl-path", type=str, required=True)
    parser.add_argument("--zip-path", type=str, required=True)
    parser.add_argument("--image-code-path", type=str, required=True)
    ...
    parser.add_argument("--reconstruction-loss", type=str, default='l1', help="reconstruction loss type of image pixel")
