...
class VQModel(nn.Module):
    def __init__(self, config: ModelArgs):
        ...
        # self.decoder = ConsistDecoder(ch_mult=config.decoder_ch_mult, z_channels=config.z_channels, dropout=config.dropout_p)
    ...
    def forward(self, input, only_quant=False, shape=None):
        if only_quant and shape is not None:
            return self.decode_code(input, shape)
...
class ConsistDecoder(nn.Module):
    def __init__(self, z_channels=256, ch=128, ch_mult=(1,1,2,2,4), num_res_blocks=2, norm_type="group",
                 dropout=0.0, resamp_with_conv=True, out_channels=3):
        super().__init__()
        self.num_resolutions = len(ch_mult)
        ...
        self.mid = nn.ModuleList()
        self.mid.append(ResnetBlock(block_in, block_in, dropout=dropout, norm_type=norm_type))
        self.mid.append(CrossAttnBlock(block_in, norm_type=norm_type))
        # self.mid.append(CrossAttnBlock(block_in, norm_type=norm_type))
        self.mid.append(ResnetBlock(block_in, block_in, dropout=dropout, norm_type=norm_type))
        ...
        for i_level in reversed(range(self.num_resolutions)):
            ...
            for _ in range(self.num_res_blocks + 1):
                ...
                block_in = block_out
                if i_level == self.num_resolutions - 1:
                    attn_block.append(CrossAttnBlock(block_in, norm_type))
                    # attn_block.append(CrossAttnBlock(block_in, norm_type))
            conv_block.res = res_block
            conv_block.attn = attn_block
            # downsample
...
class CrossAttnBlock(nn.Module):
    def __init__(self, in_channels, norm_type='group', kv_ratio=2):
        super().__init__()
    ...
    def generate_neighbor(self, x):
        ...
        for i in range(num_viewes):
            ...
            x_kv.append(torch.cat([x_left, x[:, i], x_right], dim=-2))
        # print()
        # x_kv: B, T, self.kv_ratio * N + 1, C
    ...
    def forward(self, x):
        ...
        b,c,h,w = q.shape
        q = q.reshape(b,c,h*w)
        q = q.permute(0,2,1)   # t,hw,c
        k = k.reshape(b,c,h*w).transpose(-1, -2).unsqueeze(0) # 1,t,c,hw
        k = self.generate_neighbor(k).transpose(-1, -2).squeeze(0) # t, c, kv_ratio*hw
        w_ = torch.bmm(q,k)     # b,hw,hw    w[b,i,j]=sum_c q[b,i,c]k[b,c,j]
        w_ = w_ * (int(c)**(-0.5))
        w_ = F.softmax(w_, dim=2)

        # attend to values
        v = v.reshape(b,c,h*w).transpose(-1, -2).unsqueeze(0)
        v = self.generate_neighbor(v).transpose(-1, -2).squeeze(0) # t, c, kv_ratio*hw
        w_ = w_.permute(0,2,1)   # b,hw,hw (first hw of k, second of q)
        h_ = torch.bmm(v,w_)     # b, c,hw (hw of q) h_[b,c,j] = sum_i v[b,c,i] w_[b,i,j]
        h_ = h_.reshape(b,c,h,w)
