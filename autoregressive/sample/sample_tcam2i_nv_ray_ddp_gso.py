...
def main(args):
    ...
    num_fid_samples = min(args.num_fid_samples, 30) # TODO: hard code
    ...
    for prompt in tqdm(gso_prompts):
        prompt_batch = [prompt]
        ...
        for prompt_index, prompt in enumerate(prompt_batch):
            prompt_ = prompt.replace(" ", "_").replace(".", "").replace("/", "_").replace(",", "")
            sample = samples[prompt_index]
            torchvision.utils.save_image(sample, f"{sample_folder_dir}/images/{prompt_}.png")
