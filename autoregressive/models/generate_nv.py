...
def prefill(model, cond_idx: torch.Tensor, cond_camera: torch.Tensor, input_pos: torch.Tensor, cfg_scale: float, **sampling_kwargs):
    if cfg_scale > 1.0:
        if model.model_type == "i&cam2i":
            logits, _ = model(None, cond_idx[0], cond_camera, input_pos, T=cond_camera.shape[1], prefill=True, image_condition_embed=cond_idx[1])
        else:
        ...
        logits, _ = model(None, cond_idx, cond_camera, input_pos, T=cond_camera.shape[1], prefill=True)
    if model.model_type in ['t&cam2i', 'i&cam2i', 'ti&cam2i', 't&shape_mv']:
...
def decode_one_token(model, x: torch.Tensor, cond_camera: torch.Tensor, input_pos: torch.Tensor, cfg_scale: float, cfg_flag: bool, T: int = 1, cond_idx=None, **sampling_kwargs):
    ...
    if cfg_scale > 1.0:
        ...
        if model.model_type == "i&cam2i":
            logits, _ = model(x_combined, cond_idx=None, camera_idx=cond_camera, input_pos=input_pos, T=T, image_condition_embed=cond_idx[1])
        else:
...
def generate(model, cond, max_new_tokens, cond_camera=None, emb_masks=None, cfg_scale=1.0, cfg_interval=-1, **sampling_kwargs):
    if model.model_type == 'c2i':
        ...
        if cfg_scale > 1.0:
            ...
            cond_combined = cond
        T = cond.shape[1]
    elif model.model_type == 't&cam2i':
        assert cond_camera is not None
        if cfg_scale > 1.0:
            cond_null = torch.zeros_like(cond) + model.cls_embedding.uncond_embedding
            # cond_null = torch.zeros_like(cond)
            cond_combined = torch.cat([cond, cond_null]) # 2, num_tokens
            # NOTE: here, concat null for camera pos
            cond_camera_null = torch.zeros_like(cond_camera)
            cond_camera_combined = torch.cat([cond_camera, cond_camera_null]) # 2, (num_views - 1) * num_tokens, dim
        else:
            cond_combined = cond
            cond_camera_combined = cond_camera
        T = cond.shape[1] + 1
    elif model.model_type == 'i&cam2i':
        assert cond_camera is not None
        if cfg_scale > 1.0:
            cond_null = torch.zeros_like(cond[0])
            cond_combined = torch.cat([cond[0], cond_null]) # 2, num_tokens
            reference_warp_image_null = torch.zeros_like(cond[1])
            reference_warp_image_combined = torch.cat([cond[1], reference_warp_image_null])
            cond_combined = [cond_combined, reference_warp_image_combined]
            ...
            cond_camera_combined = cond_camera
        T = cond[0].shape[1] + 1
    elif model.model_type == 'ti&cam2i':
        assert cond_camera is not None
        if cfg_scale > 1.0:
            # reference image condition cfg
            reference_image_cond = cond[0]
            reference_image_cond_null = torch.zeros_like(reference_image_cond)
            reference_image_cond_combined = torch.cat([reference_image_cond, reference_image_cond_null]) # 2, num_tokens
            # text condition cfg
            text_cond = cond[1]
            text_cond_null = torch.zeros_like(text_cond) + model.cls_embedding.uncond_embedding
            text_cond_combined = torch.cat([text_cond, text_cond_null]) # 2, num_tokens
            # conditon
            cond_combined = [reference_image_cond_combined, text_cond_combined]
            # NOTE: here, concat null for camera pos
            cond_camera_null = torch.zeros_like(cond_camera)
            cond_camera_combined = torch.cat([cond_camera, cond_camera_null]) # 2, (num_views - 1) * num_tokens, dim
        else:
            cond_combined = cond
            cond_camera_combined = cond_camera
        T = cond[0].shape[1] + cond[1].shape[1] + 1

    elif model.model_type == 't&shape_mv':
        assert cond_camera is not None
        if cfg_scale > 1.0:
            # reference image condition cfg
            reference_image_cond = cond[0]
    ...
    if model.model_type == "ti&cam2i" or model.model_type == "i&cam2i" or model.model_type == 't&shape_mv':
    ...
    if emb_masks is not None:
        ...
        if model.model_type == 't&cam2i':
            ...
            num_language = T - 1 - model.block_size
        elif  model.model_type == 't&shape_mv':
            num_language = T - 1 - 256
        assert emb_masks.shape[0] == max_batch_size
        assert emb_masks.shape[-1] == num_language

        # debug
        device = model.causal_mask.device
        ...
        if model.model_type == "ti&cam2i":
            model.causal_mask[:, :, num_language: num_language + model.block_size] = 0 # TODO
        elif model.model_type == 't&shape_mv':
            model.causal_mask[:, :, num_language: num_language + 256 + 1] = 0

        eye_matrix = torch.eye(model.causal_mask.size(1), model.causal_mask.size(2), device=device)
        model.causal_mask[:] = model.causal_mask * (1 - eye_matrix) + eye_matrix
    else:
        assert model.model_type == "i&cam2i"
        model.causal_mask[:, :, :model.block_size] = 0
        eye_matrix = torch.eye(model.causal_mask.size(1), model.causal_mask.size(2), device=device)
        model.causal_mask[:] = model.causal_mask * (1 - eye_matrix) + eye_matrix
